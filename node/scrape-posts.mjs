// Scrape Instagram posts and reels, zero dependencies (Node 18+).
//
// Usage:
//   export APIFY_TOKEN=your_token_here
//   node scrape-posts.mjs natgeo nasa
//
// Starts the actor, waits for it to finish, then reads two things: the
// dataset (the posts) and the run's OUTPUT record (why the run ended the
// way it did). An empty dataset alone is ambiguous, since "no matching
// posts" and "run refused" both produce zero items.

const ACTOR = "intropix~instagram-posts-reels-scraper";
...
async function scrapePosts(usernames, maxPosts = 10) {
  ...
  let { data: run } = await apiFetch(
    ...
    token,
    { method: "POST", body: JSON.stringify({ usernames, maxPosts }) }
  );
...
const posts = await scrapePosts(usernames.length ? usernames : ["natgeo"]);

console.log(`${posts.length} posts`);
for (const p of posts) {
  const slides = p.media?.length ?? 0;
  console.log(
    `@${p.username} [${p.post_type}] ${p.taken_at} ` +
      `likes=${p.like_count} comments=${p.comment_count} slides=${slides}`
  );
